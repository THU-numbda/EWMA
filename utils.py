...
def load_data(dataset="satimage"):
    if dataset == "satimage":
        ...
        # gamma = 5.0 for RBF and 0.1 for csRBF
        ...
        # gamma = 50.0 for RBF and 0.1 for csRBF
        ...
        # gamma = 5.0 for RBF and 0.001 for csRBF
        ...
        # gamma = 0.4 for RBF and 0.001 for csRBF 
        # feature range{0,1}
        file = addr+"mushrooms"
        X, y = load_svmlight_file(file)
        U = X.toarray()
...
def construct_csRBF(X,Y,gamma,theta=3.0,p=2.0):
    ...
    para = theta*np.sqrt(1.0/gamma)
    compactly = np.maximum(0,np.power((1-np.sqrt(euclidean_distances(X, Y, squared=True))/para),math.ceil((d+1)/p)))
